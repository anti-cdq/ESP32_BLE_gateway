...
/*            if (adv_name != NULL) {
        ...
        case ESP_GAP_SEARCH_INQ_CMPL_EVT:
    		ESP_LOGI(GATTC_TAG, "BLE scan doen");
            xEventGroupSetBits(ble_event_group, SCAN_RESULT_BIT);	//扫描结束，置位，开始对扫描到的设备进行信号强度排序
            break;
        default:
            break;
        }
...
void ble_scan_result_init(void)
{
	...
	{
		for(j=0;j<ESP_BD_ADDR_LEN;j++)
			scan_result_nodes[i].bda[j] = 0;
	}
...
void device_mac_add(esp_ble_gap_cb_param_t* scanned_dev)
{
	...
	for(i=0;i<SCAN_DEV_NODE_NUM;i++)
	{
		/* Nodes reach the MAX nodes num, not add anymore. */
		if(nodes_index == SCAN_DEV_NODE_NUM-1)
		{
			ESP_LOGI(GATTC_TAG, "Scanned device nodes stack overflowed.");
			break;
		}
		...
		if(i == nodes_index)
		{
			for(j=0;j<ESP_BD_ADDR_LEN;j++)
			{
				scan_result_nodes[i].bda[j] = scanned_dev->scan_rst.bda[j];
			}
			scan_result_nodes[i].rssi = scanned_dev->scan_rst.rssi;
			nodes_index++;
/*			ESP_LOGI(GATTC_TAG, "Node added.");*/
			break;
		}
		else
		{
			/* check if current scanned device exist. If does, stop add */
			for(j=0;j<ESP_BD_ADDR_LEN;j++)
			{
				if(scan_result_nodes[i].bda[j] != scanned_dev->scan_rst.bda[j])
					break;
			}
			if(j == ESP_BD_ADDR_LEN)
			{
				scan_result_nodes[i].rssi = scanned_dev->scan_rst.rssi;
				break;
			}
...
void ble_scan_result_print(void)
{
	...
	for(i=1;i<nodes_index;i++)
	{
		for(j=0;j<i;j++)
		{
			if(scan_result_nodes[i].rssi > scan_result_nodes[result_print_queue[j]].rssi)
			{
				for(k=i;k>j;k--)
					result_print_queue[k] = result_print_queue[k-1];
				break;
			}
		}
		if(j < i)
			result_print_queue[j] = i;
		else
			result_print_queue[i] = i;
	}

	for(i=0;i<nodes_index;i++)
	{
		sprintf(mac, "%02X:%02X:%02X:%02X:%02X:%02X",	scan_result_nodes[result_print_queue[i]].bda[0]
											 ,	scan_result_nodes[result_print_queue[i]].bda[1]
											 ,	scan_result_nodes[result_print_queue[i]].bda[2]
											 ,	scan_result_nodes[result_print_queue[i]].bda[3]
											 ,	scan_result_nodes[result_print_queue[i]].bda[4]
											 ,	scan_result_nodes[result_print_queue[i]].bda[5]);
		sprintf(print_temp, " %-18.18s | %d", mac, scan_result_nodes[result_print_queue[i]].rssi);
		printf("%s\n", print_temp);
		if(i<10)
		{
